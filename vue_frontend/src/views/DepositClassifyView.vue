<template>
  <div class="container">
    ...
    <v-btn @click="deposit_get_classified" style="margin-bottom: 2rem"
      >검색</v-btn
    >
...
const banklist = [...new Set(store.deposits.map((item) => item.kor_co_nm))]; // 은행 리스트
const periodlist = [
  ...new Set(store.depositoptions.map((item) => item.save_trm)),
]; // 기간 리스트
...
const deposit_get_classified = function () {
  axios({
    method: "post",
    url: `${API_URL}/products/deposit_get_classified/`,
    data: {
      bank: select1.value,
